...
export default function GenerationPipeline() {
  ...
  const [enforceBlacklist, setEnforceBlacklist] = useState(true);
  ...
  const handleRun = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/pipelines/generation', {
        ...
        body: JSON.stringify({ topic, tone, enforceBlacklist }),
      });
            ...
            <form onSubmit={handleRun} className="space-y-5">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 text-sm text-slate-300 cursor-pointer">
                  <input
                    type="checkbox"
                    className="rounded bg-slate-800 border-slate-700 text-blue-500 cursor-pointer"
                    checked={enforceBlacklist}
                    onChange={(e) => setEnforceBlacklist(e.target.checked)}
                    disabled={isRunning}
                  />
