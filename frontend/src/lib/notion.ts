...
export async function getLatestResearchOutputBundle(maxAgeHours = 72): Promise<ResearchOutputBundle> {
  try {
    const pages = await queryDb('LinkedIn OS — Research Outputs', 20);
    if (pages.length === 0) {
      return { context: '', hasRecent: false, ageHours: null, sections: [] };
    }

    const newest = pages[0];
    const newestCreatedAt = getDate(newest, 'Created At');
    const newestMs = newestCreatedAt ? new Date(newestCreatedAt).getTime() : Date.now();
    const ageHours = Math.max(0, Math.round((Date.now() - newestMs) / (1000 * 60 * 60)));

    const recentPages = pages.filter((p) => {
      const created = getDate(p, 'Created At');
      if (!created) return false;
      const diffHours = (Date.now() - new Date(created).getTime()) / (1000 * 60 * 60);
      return diffHours <= maxAgeHours;
    });

    const targetPages = recentPages.length > 0 ? recentPages : pages.slice(0, 6);
    const sections = Array.from(new Set(targetPages.map((p) => getText(p, 'Section')).filter(Boolean)));
    const context = targetPages
      .map((p) => `## ${getText(p, 'Section')} Research\n\n${getText(p, 'Content')}`)
      .join('\n\n---\n\n');

    return {
      context,
      hasRecent: recentPages.length > 0,
      ageHours,
      sections,
    };
  } catch {
...
export async function saveStrategyOutput(data: {
  ...
  const page = await notion.pages.create({
    ...
    properties: {
      ...
      Content:      { rich_text: [{ text: { content: cap(extractExecutiveSummary(data.content)) } }] },
      Checklist:    { rich_text: [{ text: { content: cap(extractChecklistSummary(data.checklist)) } }] },
      'Created At': { date: { start: now() } },
    },
  });

  const children = buildStrategyBlocks(data);
  await appendBlocksInBatches(page.id, children);

  await logActivity({
    title: `Content strategy built — ${data.period}`,
    description: 'Structured strategy page + execution checklist saved to Notion',
    pipeline: 'Strategy',
    type: 'success',
  });
}
...
export async function getLatestStrategyOutput(): Promise<string> {
  try {
    ...
    if (pages.length === 0) return '';
    const p = pages[0];
    const fullText = await extractPageTextFromBlocks(p.id);
    if (fullText.trim().length > 0) return fullText.slice(0, 12000);
...
function extractExecutiveSummary(text: string): string {
  ...
  const firstParagraph = lines.find((l) => !l.startsWith('#') && !l.startsWith('-') && !/^\d+\./.test(l));
  return firstParagraph ? firstParagraph.slice(0, 600) : text.slice(0, 600);
}

function extractChecklistSummary(text: string): string {
  const items = text
    .split('\n')
    .map((l) => l.trim())
    .filter((l) => l.startsWith('- [ ]') || l.startsWith('- [x]') || l.startsWith('-'));
  if (items.length === 0) return text.slice(0, 600);
  return items.slice(0, 8).join(' | ').slice(0, 1200);
}

function rtDefault(content: string, bold = false) {
  return {
    type: 'text' as const,
    text: { content },
    annotations: {
...
function splitToChunks(text: string, max = 1800): string[] {
...
function splitIntoParagraphs(text: string): string[] {
  // Preserve explicit paragraph breaks, otherwise split long single-line text
  // into readable paragraphs by grouping sentences.
  const normalized = text.replace(/\r\n/g, '\n').trim();
  if (!normalized) return [];

  // Split on two-or-more newlines to preserve user-created paragraphs
  const explicit = normalized.split(/\n\s*\n/).map((p) => p.trim()).filter(Boolean);
  if (explicit.length > 1) return explicit;

  // If there are no explicit paragraphs, split into sentences and group.
  const sentenceRe = /[^.!?]+[.!?]+(?:\s|$)|[^.!?]+$/g;
  const sentences = normalized.match(sentenceRe) || [normalized];
  const groups: string[] = [];
  const perGroup = 2; // sentences per paragraph for readability
...
function markdownToBlocks(markdown: string): any[] {
  ...
  for (const para of paragraphs) {
    ...
    // If paragraph starts with common heading markers, handle them directly
    if (trimmed.startsWith('### ')) {
      blocks.push({ object: 'block', type: 'heading_3', heading_3: { rich_text: [rtDefault(trimmed.slice(4))] } });
      continue;
    }
    if (trimmed.startsWith('## ')) {
      blocks.push({ object: 'block', type: 'heading_2', heading_2: { rich_text: [rtDefault(trimmed.slice(3))] } });
      continue;
    }
    if (trimmed.startsWith('# ')) {
      blocks.push({ object: 'block', type: 'heading_1', heading_1: { rich_text: [rtDefault(trimmed.slice(2))] } });
      continue;
    }

    // Lists and checkboxes: process per-line within paragraph
    ...
    for (const line of lines) {
      ...
      if (line.startsWith('- [ ] ')) {
        blocks.push({ object: 'block', type: 'to_do', to_do: { rich_text: [rtDefault(line.slice(6))], checked: false } });
        continue;
      }
      if (line.startsWith('- [x] ') || line.startsWith('- [X] ')) {
        blocks.push({ object: 'block', type: 'to_do', to_do: { rich_text: [rtDefault(line.slice(6))], checked: true } });
        continue;
      }
      if (line.startsWith('- ')) {
        blocks.push({ object: 'block', type: 'bulleted_list_item', bulleted_list_item: { rich_text: [rtDefault(line.slice(2))] } });
        continue;
      }
      if (/^\d+\.\s+/.test(line)) {
        blocks.push({ object: 'block', type: 'numbered_list_item', numbered_list_item: { rich_text: [rtDefault(line.replace(/^\d+\.\s+/, ''))] } });
        continue;
      }
      if (line.startsWith('> ')) {
        blocks.push({ object: 'block', type: 'quote', quote: { rich_text: [rtDefault(line.slice(2))] } });
        continue;
      }

      // Fallback: long paragraph split into chunks for Notion
      for (const chunk of splitToChunks(line)) {
        blocks.push({ object: 'block', type: 'paragraph', paragraph: { rich_text: [rtDefault(chunk)] } });
      }
    }
  }
...
function buildStrategyBlocks(data: {
  ...
  const blocks: any[] = [
    {
      ...
      callout: {
        ...
        rich_text: [rtDefault(`Content Strategy (${data.period}) generated by Pipeline 2 with research-backed planning and execution tasks.`)],
      },
    },
    {
      object: 'block',
      ...
      callout: {
        ...
        rich_text: [rtDefault(data.sourceSummary ?? 'Research source summary not provided.')],
      },
    },
    { object: 'block', type: 'divider', divider: {} },
    { object: 'block', type: 'table_of_contents', table_of_contents: { color: 'default' } },
    { object: 'block', type: 'divider', divider: {} },
    { object: 'block', type: 'heading_2', heading_2: { rich_text: [rtDefault('Executive Snapshot')] } },
  ];

  const overviewText = data.researchOverview
    ? data.researchOverview
    : 'Research overview was not generated in this run.';
  // Build an executive snapshot with quick-bullets for faster scanning
  const sentenceRe = /[^.!?]+[.!?]+(?:\s|$)|[^.!?]+$/g;
  ...
  if (bullets.length > 0) {
    ...
    for (const b of bullets) {
      blocks.push({ object: 'block', type: 'bulleted_list_item', bulleted_list_item: { rich_text: [rtDefault(b)] } });
    }
    // divider to separate quick-scan and detail
    blocks.push({ object: 'block', type: 'divider', divider: {} });
  }

  // Append the full overview (split into readable paragraphs)
...
async function appendBlocksInBatches(pageId: string, blocks: any[]): Promise<void> {
  const batchSize = 90;
...
// ─── Research Sprint (rich structured Notion pages) ───────────────────────

export interface ResearchSprintResult {
  masterPageId: string;
  masterUrl: string;
  childUrls: { market: string; audience: string; competitor: string; summary: string };
}
...
function parseInlineRichText(text: string): any[] {
  ...
  while ((match = regex.exec(text)) !== null) {
    if (match.index > lastIndex) parts.push(rtDefault(text.slice(lastIndex, match.index)));
    parts.push(rtDefault(match[1], true));
    lastIndex = match.index + match[0].length;
  }
  if (lastIndex < text.length) parts.push(rtDefault(text.slice(lastIndex)));
  return parts.length > 0 ? parts : [rtDefault(text)];
}

function buildTableBlockFromMarkdownLines(lines: string[]): any | null {
  const dataRows = lines.filter(l => !/^\|[\s\-:|]+\|$/.test(l));
  if (dataRows.length < 2) return null;
  const parseRow = (line: string) => line.split('|').slice(1, -1).map(c => c.trim());
  const headers = parseRow(dataRows[0]);
  const tableWidth = Math.max(1, headers.length);
  const normalize = (cells: string[]) => {
    while (cells.length < tableWidth) cells.push('');
    return cells.slice(0, tableWidth).map(c => [rtDefault(c)]);
  };
  return {
    ...
    table: {
      ...
      children: [
        { object: 'block', type: 'table_row', table_row: { cells: headers.map(h => [rtDefault(h, true)]) } },
        ...dataRows.slice(1).map(row => ({
          object: 'block', type: 'table_row',
          table_row: { cells: normalize(parseRow(row)) },
        })),
      ],
    },
  };
...
function markdownToRichBlocks(markdown: string): any[] {
  const blocks: any[] = [];
  ...
  const flushTable = () => {
    if (tableBuffer.length >= 2) {
      const tbl = buildTableBlockFromMarkdownLines(tableBuffer);
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith('### ')) { blocks.push({ object: 'block', type: 'heading_3', heading_3: { rich_text: parseInlineRichText(line.slice(4)) } }); continue; }
    if (line.startsWith('## ')) { blocks.push({ object: 'block', type: 'heading_2', heading_2: { rich_text: parseInlineRichText(line.slice(3)) } }); continue; }
    if (line.startsWith('# ')) { blocks.push({ object: 'block', type: 'heading_1', heading_1: { rich_text: parseInlineRichText(line.slice(2)) } }); continue; }
    if (line.startsWith('- [ ] ')) { blocks.push({ object: 'block', type: 'to_do', to_do: { rich_text: parseInlineRichText(line.slice(6)), checked: false } }); continue; }
    if (line.startsWith('- [x] ') || line.startsWith('- [X] ')) { blocks.push({ object: 'block', type: 'to_do', to_do: { rich_text: parseInlineRichText(line.slice(6)), checked: true } }); continue; }
    if (line.startsWith('- ')) { blocks.push({ object: 'block', type: 'bulleted_list_item', bulleted_list_item: { rich_text: parseInlineRichText(line.slice(2)) } }); continue; }
    if (/^\d+\.\s+/.test(line)) { blocks.push({ object: 'block', type: 'numbered_list_item', numbered_list_item: { rich_text: parseInlineRichText(line.replace(/^\d+\.\s+/, '')) } }); continue; }
    if (line.startsWith('> ')) { blocks.push({ object: 'block', type: 'quote', quote: { rich_text: parseInlineRichText(line.slice(2)) } }); continue; }
    for (const chunk of splitToChunks(line)) {
      blocks.push({ object: 'block', type: 'paragraph', paragraph: { rich_text: parseInlineRichText(chunk) } });
    }
  }
  if (tableBuffer.length > 0) flushTable();
  return blocks;
...
async function createResearchChildPage(parentId: string, emoji: string, title: string, content: string): Promise<string> {
  const page = await notion.pages.create({
    parent: { type: 'page_id', page_id: parentId },
    icon: { type: 'emoji', emoji: emoji as any },
    properties: { title: { title: [{ text: { content: title } }] } },
  });
  const blocks = markdownToRichBlocks(content).slice(0, 400);
  await appendBlocksInBatches(page.id, blocks);
  return page.id;
}

function buildSprintMasterBlocks(data: {
  niche: string; icpSummary: string; competitors: string;
  totalQueries: number; executiveSummary: string;
}, dateStr: string): any[] {
  const mkRow = (a: string, b: string) => ({
    object: 'block', type: 'table_row',
    table_row: { cells: [[rtDefault(a)], [rtDefault(b)]] },
  });
  const hdr = (a: string, b: string) => ({
    object: 'block', type: 'table_row',
    table_row: { cells: [[rtDefault(a, true)], [rtDefault(b, true)]] },
  });

  const blocks: any[] = [
    { object: 'block', type: 'heading_2', heading_2: { rich_text: [rtDefault('Research Sprint Overview')] } },
    {
      object: 'block', type: 'table',
      table: {
        ...
        children: [
          hdr('Field', 'Value'),
          mkRow('Run Date', dateStr),
          mkRow('Model', 'Perplexity Sonar Pro + Claude Sonnet'),
          mkRow('Total Queries', String(data.totalQueries)),
          mkRow('Status', 'COMPLETE ✅'),
        ],
      },
    },
    { object: 'block', type: 'divider', divider: {} },
    { object: 'block', type: 'heading_2', heading_2: { rich_text: [rtDefault('Sprint Context')] } },
    {
      object: 'block', type: 'table',
      table: {
        table_width: 2, has_column_header: true, has_row_header: false,
        children: [
          hdr('Field', 'Value'),
          mkRow('Niche', data.niche.slice(0, 500)),
          mkRow('ICP', data.icpSummary.slice(0, 500)),
          mkRow('Competitors', data.competitors || 'Not specified'),
        ],
      },
    },
    { object: 'block', type: 'divider', divider: {} },
    { object: 'block', type: 'heading_2', heading_2: { rich_text: [rtDefault('Sub-Pages')] } },
    { object: 'block', type: 'paragraph', paragraph: { rich_text: [rtDefault('Each detailed report is available as a dedicated sub-page below:')] } },
    { object: 'block', type: 'bulleted_list_item', bulleted_list_item: { rich_text: [rtDefault('📈 Market Research', true), rtDefault(' — Algorithm, formats, trends, pricing, growth tactics')] } },
    { object: 'block', type: 'bulleted_list_item', bulleted_list_item: { rich_text: [rtDefault('🎯 Audience Research', true), rtDefault(' — Pain points, behavioral patterns, buying behavior')] } },
    { object: 'block', type: 'bulleted_list_item', bulleted_list_item: { rich_text: [rtDefault('🕵️ Competitor Intelligence', true), rtDefault(' — Per-creator profiles, gaps, positioning white space')] } },
    { object: 'block', type: 'bulleted_list_item', bulleted_list_item: { rich_text: [rtDefault('📋 Executive Summary', true), rtDefault(' — Top findings, immediate actions, 90-day plan')] } },
    { object: 'block', type: 'divider', divider: {} },
    { object: 'block', type: 'heading_2', heading_2: { rich_text: [rtDefault('Executive Summary & Key Actions')] } },
    ...markdownToRichBlocks(data.executiveSummary).slice(0, 150),
  ];

  return blocks.slice(0, 350);
}

export async function saveResearchSprint(data: {
  niche: string;
  icpSummary: string;
  marketReport: string;
  ...
  competitors: string;
  totalQueries: number;
}): Promise<ResearchSprintResult> {
  const dateStr = new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });

  const masterPage = await notion.pages.create({
    parent: { type: 'page_id', page_id: PARENT_PAGE_ID },
    icon: { type: 'emoji', emoji: '🔬' },
    properties: { title: { title: [{ text: { content: `Pipeline 1 — Research Sprint · ${dateStr}` } }] } },
  });
  const masterPageId = masterPage.id;

  await appendBlocksInBatches(masterPageId, buildSprintMasterBlocks({
    niche: data.niche, icpSummary: data.icpSummary, competitors: data.competitors,
    totalQueries: data.totalQueries, executiveSummary: data.executiveSummary,
  }, dateStr));

  const [marketId, audienceId, competitorId, summaryId] = await Promise.all([
    createResearchChildPage(masterPageId, '📈', `Market Research — ${dateStr}`, data.marketReport),
    createResearchChildPage(masterPageId, '🎯', `Audience Research — ${dateStr}`, data.audienceReport),
    createResearchChildPage(masterPageId, '🕵️', `Competitor Intelligence — ${dateStr}`, data.competitorReport),
    createResearchChildPage(masterPageId, '📋', `Executive Summary — ${dateStr}`, data.executiveSummary),
  ]);

  return {
    masterPageId,
    masterUrl: pageUrl(masterPageId),
    childUrls: {
      market: pageUrl(marketId),
      audience: pageUrl(audienceId),
      competitor: pageUrl(competitorId),
      summary: pageUrl(summaryId),
    },
  };
}
...
function blockRichTextPlain(block: any): string {
  const container = block?.[block.type];
  if (!container || !Array.isArray(container.rich_text)) return '';
  return container.rich_text.map((r: any) => r.plain_text ?? '').join('');
}
...
async function extractPageTextFromBlocks(pageId: string, depth = 0): Promise<string> {
  if (depth > 2) return '';
  ...
  for (const block of blocks) {
    const text = blockRichTextPlain(block).trim();
    const type = block.type;

    if (type === 'heading_1' && text) lines.push(`# ${text}`);
    else if (type === 'heading_2' && text) lines.push(`## ${text}`);
    else if (type === 'heading_3' && text) lines.push(`### ${text}`);
    else if (type === 'paragraph' && text) lines.push(text);
    else if (type === 'bulleted_list_item' && text) lines.push(`- ${text}`);
    else if (type === 'numbered_list_item' && text) lines.push(`1. ${text}`);
    else if (type === 'to_do' && text) {
      const checked = Boolean(block?.to_do?.checked);
      lines.push(`- [${checked ? 'x' : ' '}] ${text}`);
    } else if (type === 'quote' && text) lines.push(`> ${text}`);

    if (block.has_children) {
      const nested = await extractPageTextFromBlocks(block.id, depth + 1);
